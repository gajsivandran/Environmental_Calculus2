...
df <- read_tsv(
  ...
  mutate(
    rain_cm = replace_na(rain_cm, 0),
    hour = parse_number(time) # "9:53" -> 9
  )

# Build an ordered time index WITHOUT real dates:
# larger day = later; within day, larger hour = later.
df <- df %>%
  mutate(order_key = day * 24 + hour) %>%
  arrange(order_key)

# Make a complete hourly sequence in that order_key space
full <- tibble(order_key = seq(min(df$order_key), max(df$order_key), by = 1)) %>%
  left_join(df %>% select(order_key, rain_cm), by = "order_key") %>%
  mutate(rain_cm = replace_na(rain_cm, 0))

# Create a pseudo-time for plotting (hours since start)
full <- full %>%
  mutate(hours_since_start = order_key - min(order_key))
