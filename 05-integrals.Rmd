...
Because the factor \((x - 1)\) appears with power 2, we include **one term for each power**:
\[
\frac{1}{(x - 1)^2}
=
...
Because the equation holds for all \(x\), we are free to choose values of \(x\) that simplify the algebra.

---

##### Why Choose \(x = 1\)?

We choose \(x = 1\) because it **zeros out one of the terms**.
...
Since \(1 - 1 = 0\), the term involving \(A\) disappears:
\[
1 = 0 + B.
\]
...
This isolates one coefficient without solving a system of equations.

---

#### Step 4: Solve for the Remaining Coefficient

Substitute \(B = 1\) back into the equation:
\[
1 = A(x - 1) + 1.
\]
...
\frac{1}{(x - 1)^2}
=
\frac{0}{x - 1} + \frac{1}{(x - 1)^2}
=
\frac{1}{(x - 1)^2}.
\]

In this case, the algebra confirms that no additional simplification is needed.
...
- **every power must appear in the decomposition**,
- choosing values that zero out factors simplifies coefficient solving,
- once decomposed, each term integrates using familiar rules.

This case reinforces the core strategy of partial fractions:  
